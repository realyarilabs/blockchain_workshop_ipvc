var livePatern = {
  ...
  cols: 0,
  rows: 0,
  colors: [210, 20, 249, 248, 241, 240],
  triangleColors: [],
  destColors: [],
  live: true,

  init: function(isLive = true) {
    this.live = isLive;
    this.canvas = document.createElement('canvas');
  
    this.context = this.canvas.getContext('2d');
    this.cols = Math.floor(document.body.clientWidth / 11);
  ...
  animate: function(){
    ...
    setTimeout(() => {    
      me.animate.call(me);
      if (me.live) {
        document.body.style.background = 'url(' + me.canvas .toDataURL() + ')';
      }
    }, 100);
